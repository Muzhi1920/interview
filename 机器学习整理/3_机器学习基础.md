...
- [机器学习基础](#机器学习基础)
  - [一、模型](#一模型)
    - [1、生成式模型](#1生成式模型)
    - [2、判别式模型](#2判别式模型)
    - [3、模型的泛化能力-泛化误差上界](#3模型的泛化能力-泛化误差上界)
    - [4、模型融合](#4模型融合)
  - [二、策略](#二策略)
    - [1、Loss 损失函数](#1loss-损失函数)
    - [2、评价指标](#2评价指标)
    - [3、ROC曲线](#3roc曲线)
      - [3.1、AUC](#31auc)
      - [3.2、AUC值的物理意义](#32auc值的物理意义)
      - [3.3、AUC值的计算](#33auc值的计算)
      - [3.4、ROC与P-R曲线对比](#34roc与p-r曲线对比)
    - [4、排序指标](#4排序指标)
      - [MAP](#map)
      - [举个列子](#举个列子)
      - [NDCG](#ndcg)
  - [三、方法](#三方法)
    - [1、机器学习中的梯度下降](#1机器学习中的梯度下降)
      - [1.1、批梯度下降比较](#11批梯度下降比较)
      - [1.2、最速下降](#12最速下降)
      - [1.3、多分类梯度下降](#13多分类梯度下降)
    - [2、牛顿法](#2牛顿法)
    - [3.Levenberg-Marquardt修正](#3levenberg-marquardt修正)
      - [牛顿法 vs 梯度下降](#牛顿法-vs-梯度下降)
    - [4.拟牛顿法-牛顿法hessian矩阵的优化求解](#4拟牛顿法-牛顿法hessian矩阵的优化求解)
  - [四、偏差与方差](#四偏差与方差)
    - [4.1、方差与偏差推导](#41方差与偏差推导)
  - [五、L1和L2正则](#五l1和l2正则)
    - [解的稀疏性](#解的稀疏性)
  - [六、优化过拟合和欠拟合](#六优化过拟合和欠拟合)
    - [降低过拟合](#降低过拟合)
    - [降低欠拟合](#降低欠拟合)
  - [七、拉格朗日乘法](#七拉格朗日乘法)
    - [7.1、KKT条件](#71kkt条件)
  - [八、特征选择](#八特征选择)
    - [为何标准化和归一化](#为何标准化和归一化)
      ...
      - [无量纲化](#无量纲化)
      - [避免数值bias](#避免数值bias)
    - [标准化：更好保持样本间距；符合统计假设](#标准化更好保持样本间距符合统计假设)
      - [什么时候需要标准化](#什么时候需要标准化)
    - [归一化-广义上也可以指标准化](#归一化-广义上也可以指标准化)
    - [何时归一化，何时标准化](#何时归一化何时标准化)
    - [特征选取方法-理论](#特征选取方法-理论)
  - [数据不平衡](#数据不平衡)
...
<a id="markdown-一模型" name="一模型"></a>
## 一、模型
<a id="markdown-1生成式模型" name="1生成式模型"></a>
### 1、生成式模型
由数据学到联合概率分布$P(X,Y)$,然后求出条件概率分布$P(Y|X)$作为预测模型。给定输入X产生输出Y的生成关系。**收敛更快，隐变量时仍然能用**
$$P(Y|X)=\frac{P(X,Y)}{P(X)}$$

>>朴素贝叶斯、隐马尔科夫模型（HMM）。

<a id="markdown-2判别式模型" name="2判别式模型"></a>
### 2、判别式模型
由数据学习到决策函数f(X)或者条件概率模型P(Y|X)作为预测的模型。给定输入 X，预测什么样的输出Y。**更为准确，隐变量失效，定义和使用特征**
>>KNN、感知机、逻辑斯蒂回归（Logistic Regression）、最大熵、支持向量机（SVM）、条件随机场（CRF）、决策树。

<a id="markdown-3模型的泛化能力-泛化误差上界" name="3模型的泛化能力-泛化误差上界"></a>
### 3、模型的泛化能力-泛化误差上界
...
<a id="markdown-4模型融合" name="4模型融合"></a>
### 4、模型融合
1. 平均法
2. 投票法
3. bagging
...
<a id="markdown-2评价指标" name="2评价指标"></a>
### 2、评价指标
>>准确率，精确率，召回率，均方根误差
