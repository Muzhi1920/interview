...
1. [公式推导整理](#公式推导整理)
   1. [EM算法](#em算法)
      1. [求解含有隐变量的概率模型](#求解含有隐变量的概率模型)
      2. [收敛性证明](#收敛性证明)
      3. [结论](#结论)
   2. [朴素贝叶斯](#朴素贝叶斯)
         1. [朴素贝叶斯模型的求解](#朴素贝叶斯模型的求解)
   3. [GBDT vs XGBDT](#gbdt-vs-xgbdt)
      1. [目标函数](#目标函数)
         1. [决策树函数$f_k$](#决策树函数f_k)
         2. [决策树的复杂度](#决策树的复杂度)
         3. [XGBoost的目标函数](#xgboost的目标函数)
      2. [决策树构造](#决策树构造)
   4. [$FM算法$](#fm算法)
      1. [$FM$算法小结](#fm算法小结)
   5. [L1，L2正则](#l1l2正则)
      1. [从贝叶斯先验概率看正则化](#从贝叶斯先验概率看正则化)
      2. [$L1$正则化的概率解释](#l1正则化的概率解释)
      3. [$L2$正则化的概率解释](#l2正则化的概率解释)
   6. [LSTM与GRU](#lstm与gru)
   7. [PCA降维](#pca降维)
   8. [感知机模型](#感知机模型)
   9. [BP反向传播算法](#bp反向传播算法)
      ...
      2. [输出层$i->j$](#输出层i-j)
      3. [隐藏层$j->k$](#隐藏层j-k)
   10. [回归算法与最小二乘](#回归算法与最小二乘)
      1. [总结](#总结)
   11. [K-means](#k-means)
      1. [k-means算法的损失函数](#k-means算法的损失函数)
      2. [优化损失函数](#优化损失函数)
