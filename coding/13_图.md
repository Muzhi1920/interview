...
```cpp
//松弛操作：移动邻接链表得到start->end
void Relax(Graph *graph, int start, int end){
    ENode *edge =graph->Edge[start]->first_edge;
    while (graph->Edge[edge->Node]!= graph->Edge[end]){
        edge = edge->next;
    }
    if (graph->Edge[edge->Node] == graph->Edge[end]){
        if (graph->Edge[end]->value > graph->Edge[start]->value + edge->weight){
            graph->Edge[end]->value = graph->Edge[start]->value + edge->weight;
            graph->Edge[end]->pre = start;
        }
    }
}
...
int getMin(Graph *graph){
    ...
    for (int i = 1; i <= graph->vexnum; i++){
        if (graph->Edge[i]->category != False){
            if (graph->Edge[i]->value < min){
                min = graph->Edge[i]->value;
                min_node = i;
            }
        }
    }
    if (min_node == 0)
        return 0;
    graph->Edge[min_node]->category = False;
    return min_node;
}
//D算法，整体为BFS，邻接链表
void Dijkstra(Graph *graph, int start){
    graph->Edge[start]->category = False;//获取节点
    ENode *edge = graph->Edge[start]->first_edge;//遍历源节点外一层，对其松弛
    while (edge != NULL){
        Relax(graph, start, edge->Node);//该边指向顶点
        edge = edge->next;
    }
    //每一次选择剩余节点里面得分最小的节点，标记后对其松弛
    for (int i = 0; i < graph->vexnum - 1; i++){
        int u = getMin(graph);//遍历O(N)得到d最小的节点编号
        if (u == 0) return;//无可取则返回
        edge = graph->Edge[u]->first_edge; //遍历u节点相邻的点对其松弛
        while (edge != NULL){
            Relax(graph, u, edge->Node);
            edge = edge->next;
        }
    }
}
