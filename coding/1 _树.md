...
while(!Q.empty())：
    res(Q.back())；
    for(Q.size())：//队列保留该层的元素
        d=Q.top();Q.pop();将左右入队列
        if(d.left):Q(left)
        if(d.right):Q(right)
```
- 子树
...
f(s,t):
    if(!s):return false；
    if g(s,t): return true;
    return g(sleft,t) || g(sright,t)
...
int help(node):
    !node?:0；
    l=help(left);if(l==-1):return -1;
    r=help(right);if(r==-1):return -1;
...
f(node):
    ...
    while(!right):right->right;l++;  //计算左右深度
    if(l==r):N=(2^d)-1
    else：
