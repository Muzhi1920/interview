...
- [动态规划](#动态规划)
    ...
    - [一维动规：数组-典型子问题](#一维动规数组-典型子问题)
        ...
        - [最少加油次数-贪心](#最少加油次数-贪心)
        - [第N个丑数——累加](#第n个丑数累加)
        - [等差递增子区间个数——累加](#等差递增子区间个数累加)
        - [最大乘积子序列——累加](#最大乘积子序列累加)
        - [分割整数使得乘积最大——子问题重复](#分割整数使得乘积最大子问题重复)
            - [动态规划](#动态规划-1)
            - [贪心算法](#贪心算法)
        - [分割整数为平方数加和——子问题重复](#分割整数为平方数加和子问题重复)
        - [26字母编码解码种类数——累加](#26字母编码解码种类数累加)
        - [最长摆动子序列[返回摆动次数]——累加](#最长摆动子序列返回摆动次数累加)
        - [0-1背包问题](#0-1背包问题)
        - [数组元素加正负使其总和为target，返回次数。——累加](#数组元素加正负使其总和为target返回次数累加)
...
<a id="markdown-矩阵的最小大路径和扩增" name="矩阵的最小大路径和扩增"></a>
### 矩阵的最小（大）路径和——扩增
...
<a id="markdown-循环数字最后" name="循环数字最后"></a>
### 循环数字最后
f(n-1)+m %  n\
去除数字后，再次映射0--n-1；p(x)=(x-k-1)%m;逆映射p-1(x)=(x+k+1)%m;k=(m-1)%n;k+1=(m-1)%n+1=m
...
<a id="markdown-动态规划-1" name="动态规划-1"></a>
#### 动态规划
```cpp
vector<int>dp(n+1);
dp[1] = 1;
...
return dp[n];
```
<a id="markdown-贪心算法" name="贪心算法"></a>
#### 贪心算法
```cpp
int fun2(int n){
    if(n < 2)
