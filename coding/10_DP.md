...
- [动态规划](#动态规划)
  ...
  - [一维动规：数组](#一维动规数组)
    ...
    - [最长摆动子序列](#最长摆动子序列)
    - [0-1背包问题](#0-1背包问题)
    - [改变子数组正负使其和为某target值](#改变子数组正负使其和为某target值)

<!-- /TOC -->
<a id="markdown-矩阵问题" name="矩阵问题"></a>
## 矩阵问题
DFS,BFS，$N^2$技巧遍历
<a id="markdown-最大子数组和最大子矩阵和" name="最大子数组和最大子矩阵和"></a>
### 最大子数组和，最大子矩阵和
>>网易
...
<a id="markdown-求取子矩阵的和" name="求取子矩阵的和"></a>
### 求取子矩阵的和
>>给定位置得到和
- 加和
...
<a id="markdown-最大正方形" name="最大正方形"></a>
### 最大正方形
...
<a id="markdown-矩阵的最小路径和" name="矩阵的最小路径和"></a>
### 矩阵的最小路径和
```cpp
//先对首行处理
for(int j=1;j < n;j++){
...
```
<a id="markdown-二维矩阵的搜索" name="二维矩阵的搜索"></a>
### 二维矩阵的搜索
右上角出发：O(M+N)
二分：O(logM+logN)
...
for(int step=1;step<=n-1;step++){
    for(int i=1;i<=n-step;i++){
        ...
        int minx=INT_MAX;
        //类似F算法，i-k-j，遍历中间的k使[i,k],[k,j]最小
        for(int k=i;k<=j-1;k++){
...
## 二维动规：字符串
<a id="markdown-最长公共子序列" name="最长公共子序列"></a>
### 最长公共子序列
...
<a id="markdown-最小编辑距离" name="最小编辑距离"></a>
### 最小编辑距离
```cpp
...
<a id="markdown-一维动规数组" name="一维动规数组"></a>
## 一维动规：数组
<a id="markdown-最少加油次数" name="最少加油次数"></a>
### 最少加油次数
```cpp
...
<a id="markdown-第n个丑数给定几个数定义为丑数所有的数都是基于丑数的累乘" name="第n个丑数给定几个数定义为丑数所有的数都是基于丑数的累乘"></a>
...
<a id="markdown-等差递增子区间个数" name="等差递增子区间个数"></a>
### 等差递增子区间个数
```cpp
dp[2] = 1
    [0, 1, 2]
...
<a id="markdown-最大乘积子序列" name="最大乘积子序列"></a>
### 最大乘积子序列
```cpp
...
<a id="markdown-分割整数使得乘积最大" name="分割整数使得乘积最大"></a>
### 分割整数使得乘积最大
...
<a id="markdown-分割整数为平方数加和" name="分割整数为平方数加和"></a>
### 分割整数为平方数加和
```cpp
...
for (int i = 1; i <= n; i++) {
    ...
    for (int square : squareList) {
        if (square > i)
            break;
        dp[i] = min(dp[i], dp[i - square] + 1);//与以解决子问题取较小
    }
}
return dp[n];
```

<a id="markdown-26字母编码解码种类数" name="26字母编码解码种类数"></a>
### 26字母编码解码种类数
```cpp
...
<a id="markdown-最长摆动子序列" name="最长摆动子序列"></a>
### 最长摆动子序列
```cpp
int up = 1, down = 1;
for (int i = 1; i < nums.size(); i++) {
...
<a id="markdown-改变子数组正负使其和为某target值" name="改变子数组正负使其和为某target值"></a>
### 改变子数组正负使其和为某target值
```cpp
int sum = sum(nums);
